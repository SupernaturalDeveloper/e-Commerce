    ...
    <div id="allMenuWrapper">
        <div class="menuLists" ref="menuLists" v-show="isShowMenuList">
            <div class="menuWrapper">
                <span
                    class="menuCategoryTitle"
                    v-if="recentlyChooseLists.length > 0"
                    >{{ $t("eat.recent") }}</span
                >
                <ul class="contentTip">
                    <li
                        v-for="(item, index) in recentlyChooseLists"
                        :key="item.id"
                        class="item"
                    >
                        {{ item.name }}
        ...
        methods: {
            menuTitleClick(index) {
                ...
                if (this.recentlyChooseLists.length <= 7) {
                    // 3.1 添加到数组的第一个
                    this.recentlyChooseLists.unshift(
                        this.todayMenuCategoryLists[index]
                    );
                    this.recentlyChooseLists = Array.from(
                        new Set(this.recentlyChooseLists)
                    );
                } else {
                    // 3.2 先末尾移除一个
